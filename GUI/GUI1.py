...
class YoutubeDownloader:
    ...
    def __init__(self):
        ...
        self.path_entry.grid(column=1, row=3)
        self.ext_entry = tk.Entry(master=self.window, width=40)
        self.ext_entry.grid(column=1, row=4)
        self.download_button = tk.Button(
            self.window, text="Download", command=self.__get_link)
    ...
    def __downloader(self, link, save_path="", save_name="", extension="mp4"):
        ...
        yt_stream = yt.streams.filter(
            progressive=True, file_extension=extension).order_by('resolution').desc().first()
